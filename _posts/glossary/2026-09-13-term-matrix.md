...
[10회차 — 기계가 배운다는 것]({{ site.baseurl }}/2026/09/10/how-machines-learn/)에서 스칼라·벡터·행렬·텐서 사다리의 한 단으로 정식으로 다뤘다. 손글씨 숫자 한 장이 흑백 사진이라 28×28 행렬이고, 이 격자가 784개 밝기 숫자로 펴진다는 것도 여기서 배웠다.

다만 그전에 **1회차**에서 LLM이 무엇인지 배울 때, "대량의 텍스트를 학습해 패턴을 익힌다"는 그 학습과 계산이 전부 행렬 곱으로 이뤄진다. 겉으로 드러나지 않았을 뿐 바닥에 깔려 있던 개념이다.
